 ...
 * @property {Array=[]} overTime 超时显示文本，[{expires:1000,text:"已经很久了哦"}]
...
function getMyPath() {
    ...
    var url = scriptSrc.replace(jsName, '');
    ...
    _($.fn, {
        loading: function () {
            var nodeNames = 'BODY,HTML,#document,undefined'
                ...
                , cssLoadingClassName = 'loading' // 注：type:'css' 使用
                ...
                , imgUrl = MyUrl + 'image/loading/loading'
                , cssUrl = MyUrl + 'css/loading/loading.css'
                , imgSrcArr = [
                    imgUrl + '.gif',
                    imgUrl + '-bars.gif'
                ...
                , pteMethods = {
                    resize: function () {
                        ...
                        var offsetTopDef = 0;
                        if (this.settings.type == "css" && this.settings.text) {
                            offsetTopDef = -50;
                        }
                        var offsetTop = this.settings && (this.settings.offsetTop || offsetTopDef);
            ...
            _(Class.prototype, {
                    ...
                    if (!this.hasNewSetting && $loadingObjSingle.size() > 0) {
                        ...
                        this.destroy('auto'), ds.loadImage(hasImgSrc ? settings.imgSrc : null, function (imgObj) {
                            ...
                            if (settings.type == 'gif') {
                                ...
                                var $cssDivItem = $('<div></div>');
                                $cssDiv.addClass("div-" + cssLoadingClassName).addClass(loadingClassName).hide();
                                $cssDivItem.addClass(cssLoadingClassName + "-" + settings.cssType);

                                $cssDiv.append($cssDivItem).appendTo($this)
                            }
                            $text
                                .addClass(loadingClassName)
                            ...
                            if (overTime.length > 0) {
                                setOverTime(0);
                                function setOverTime(i) {
                                    var expires = overTime[i].expires;
                                    if (i > 0) {
                                        expires = overTime[i].expires - overTime[i - 1].expires;
                                    }
                                    setTimeout(function () {
                                        $text.html(overTime[i].text)
                            ...
                            setTimeout(function () {
                                ...
                                $cssDiv[settings.type != 'css' ? 'remove' : 'show']();
                ...
                isHide: function () {
                    var settings = this.settings;
                    var result = true;
                    var $elem = this.$this.children('.' + loadingClassName);
                    if ($elem.length > 0) {
                        if ($elem.is(':visible')) {
                            result = false;
