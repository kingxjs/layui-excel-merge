...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function (exports) {
                    ...
                    that.eachCols(function (i3, item3) {
                        ...
                        if (treeShowName == item3.field) {//当前是否用于显示的值
                            ...
                            if (o.children && o.children.length > 0) {//非叶子节点
                                ...
                                treeImgHtml += nbspHtml + '<i class="layui-icon">&nbsp;&nbsp;</i> ';
                            }
    ...
    Class.prototype.sort = function (th, type, pull, formEvent) {
        ...
        if (type === 'asc') { //升序
            ...
            thisData = layui.sort(data, options.treeUpId);
        } else if (type === 'desc') { //降序
            thisData = layui.sort(data, field, true);
            thisData = layui.sort(data, options.treeUpId, true);
        } else { //清除排序
            thisData = layui.sort(data, table.config.indexName);
            delete that.sortKey;
        }
        var thisData = that.setSortData(thisData, options.treeRootId);
    ...
    Class.prototype.setSortData = function (data, treeRootId) {
        var that = this
            , options = that.config;
        ...
        for (var i = 0; i < data.length; i++) {
            var res = data[i];
            if (res[options.treeUpId] == treeRootId) {
                sortData.push(res)
                var sortData2 = that.setSortData(data, res[options.treeId]);
                sortData.push.apply(sortData, sortData2);
            } else {
                continue;
            }
        }
