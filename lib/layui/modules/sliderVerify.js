...
layui.define(['jquery', 'layer', 'form'], function (exports) {
    ...
    var $ = layui.jquery,
        ...
        , sliderVerify = {
            read: function () {
                var css = `.slider-item{height:38px;line-height:38px;position:relative;border: 1px solid #DCDEE0;}.slider-bg{position:absolute;width:40px;height:100%;z-index:100;overflow: hidden;z-index: 121;}.slider-btn{width:40px;height:99%;position:absolute;border-right:1px solid #ccc;cursor:move;text-align:center;background-color:#fff;user-select:none;color:#666;z-index:122}.slider-btn-success{font-size:24px}.slider-text-success{color:#fff !important}.slider-text,.slider-text-success{position:absolute;text-align:center;width:100%;height:100%;user-select:none;font-size:14px;color:#000;z-index:120}.slider-error{animation:glow 800ms ease-out infinite alternate;}@keyframes glow{0%{border-color:#e6e6e6}100%{border-color:#1e9fff}}`,
    ...
    Class.prototype.render = function () {
        ...
        var sliderDom = $(`<div id="${option.domid}" ${option.isAutoVerify ? 'lay-verify="sliderVerify"' : ''} class="slider-item">
  								<div class="${MOD_BG} ${option.bg}"><div class="${MOD_TEXT_SUCCESS} ${MOD_TEXT}">${option.text}</div></div>
  								<div class="${MOD_TEXT}">${option.text}</div>
  								<div class="${MOD_BTN} layui-icon layui-icon-next"></div>
  							</div>`);
        elem.hide().after(sliderDom);
        var width = $('#' + option.domid).width();
        $("." + MOD_TEXT_SUCCESS).css("width", width + "px")
    ...
    Class.prototype.move = function (down, e) {
        ...
        if (offsetX == container.distance) {
            ...
            $(container.btn).css("box-sizing","border-box")
