...
(function (layui, window, factory) {
    if ((typeof exports === 'undefined' ? 'undefined' : _typeof(exports)) === 'object') {
        ...
        }).addcss("modules/formSelects/formSelects.css", "formSelectscss");
    } else {
    ...
    Common.prototype.renderLinkage = function (id, dataArr, linkageWidth) {
        ...
        var _loop = function _loop() {
            ...
            $.each(_temp, function (pid, arr) {
                $.each(arr, function (idx, item) {
                    ...
                    //2018年11月23日13:23:07  wxj修改
                    //if (children) {
                    //    if (children.length > 0) {
                    //        item[ajaxConfig.keyDis] = "";
                    //    }
                    //}
        ...
        $.each(result, function (idx, arr) {
            ...
            $.each(arr, function (idx2, item) {
                var span = '<li title="' + item.name + '" class="' + (item.disabled ? DISABLED : '') + '" pid="' + item.pid + '" xm-value="' + item.value + '"><span>' + item.name + '</span></li>';
                groupDiv.push(span);
            });
            groupDiv.push('</div>');
